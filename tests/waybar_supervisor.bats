...
setup() {
  ...
  mkdir -p \
    "${TEST_ROOT}/bin" \
    "${TEST_ROOT}/runtime" \
    "${TEST_ROOT}/config/waybar" \
    "${TEST_ROOT}/state" \
    "${TEST_ROOT}/home"

  # The production Waybar supervisor no longer owns Sway IPC. Its fallback
  # operations go through the shared compositor adapter, so this test fakes
  # that public boundary instead of faking swaymsg.
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    'printf "adapter %s\n" "$*" >>"${WAYBAR_TEST_EVENTS:?}"' \
    'exit 0' \
    >"${TEST_ROOT}/bin/privatestack-compositor-adapter"
  ...
  export HYPERLAB_COMPOSITOR_ADAPTER="${TEST_ROOT}/bin/privatestack-compositor-adapter"
